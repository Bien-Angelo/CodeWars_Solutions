...
The indexes of these items should then be returned in a tuple / list (depending on your language) like so: 
(index1, index2).
...
def two_sum(numbers, target):
    for index_x, x in enumerate(numbers):
        for index_y, y in enumerate(numbers):
            if x+y == target and index_x is not index_y:
                twoSumin = (index_x,index_y)
                return twoSumin
